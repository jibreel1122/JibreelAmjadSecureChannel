...
def derive_keys(shared_secret: bytes, transcript_hash: bytes) -> dict:
    ...
    client_key = hkdf(transcript_hash, shared_secret, b"client key", 32)

    print("deriving server_key")
    server_key = hkdf(transcript_hash, shared_secret, b"server key", 32)

    print("deriving client_nonce_base")
    client_nonce_base = hkdf(transcript_hash, shared_secret, b"client nonce", 12)

    print("deriving server_nonce_base")
    server_nonce_base = hkdf(transcript_hash, shared_secret, b"server nonce", 12)
