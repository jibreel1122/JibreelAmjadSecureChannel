...
def test_case(name, ikm, salt, info, length, exp_prk, exp_okm):
    ...
    prk = hkdf_extract(salt, ikm).hex()
    prk_ok = prk == exp_prk
    if prk_ok:
        print("PRK PASS")
    else:
        print("PRK FAIL")
        print("Expected:", exp_prk)
        print("Got     :", prk)

    okm = hkdf_expand(bytes.fromhex(prk), info, length).hex()
    okm_ok = okm == exp_okm
    if okm_ok:
        print("OKM PASS\n")
    else:
        print("OKM FAIL")
        print("Expected:", exp_okm)
        print("Got     :", okm)
        print()

    return prk_ok and okm_ok
...
if __name__ == "__main__":
    ...
    cases = [
        ("rfc5869 test case 1", ikm1, salt1, info1, 42,
         "077709362c2e32df0ddc3f0dc47bba6390b6c73bb50f9c3122ec844ad7c2b3e5",
         "3cb25f25faacd57a90434f64d0362f2a2d2d0a90cf1a5a4c5db02d56ecc4c5bf34007208d5b887185865"),
        ("rfc5869 test case 2", ikm2, salt2, info2, 82,
         "06a6b88c5853361a06104c9ceb35b45cef760014904671014a193f40c15fc244",
         "b11e398dc80327a1c8e7f78c596a49344f012eda2d4efad8a050cc4c19afa97c59045a99cac7827271cb41c65e590e09da3275600c2f09b8367793a9aca3db71cc30c58179ec3e87c14c01d5c1f3434f1d87"),
        ("rfc5869 test case 3 zero length salt and info", ikm1, b"", b"", 42,
         "19ef24a32c717b167f33a91d6f648bdf96596776afdb6377ac434c1c293ccb04",
         "8da4e775a563c18f715f802a063c5a31b8a11f5c5ee1879ec3454e5f3c738d2d9d201395faa4b61a96c8"),
    ]

    passed = 0
    for name, ikm, salt, info, length, exp_prk, exp_okm in cases:
        if test_case(name, ikm, salt, info, length, exp_prk, exp_okm):
            passed = passed + 1

    print("hkdf tests passed", passed, "of", len(cases))
